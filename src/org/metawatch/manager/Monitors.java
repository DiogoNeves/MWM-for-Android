...
public class Monitors {
	...
	public static void RefreshLocation() {
		if (locationUpdater==null)
			return;
		Location location = locationUpdater.getLastBestKnownLocation();
	...
	public static void stop(Context context) {
		...
		if (Preferences.weatherGeolocationMode != GeolocationMode.MANUAL & locationUpdater!=null) {
			if (locationUpdater!=null) {
	...
	private static void createLocationReceiver(Context context) {
		if (locationUpdater == null) return;
	
		locationReceiver = new BroadcastReceiver() {
	
			@Override
			public void onReceive(Context context, Intent intent) {
				if (!intent.hasExtra(LocationUpdater.KEY_LOCATION_CHANGED)) return;
				
				try {
					Location location = (Location) intent.getExtras().get(LocationUpdater.KEY_LOCATION_CHANGED);
